...
const StatCard: React.FC<Props> = ({ label, value, trend, variant = "neutral", currencySymbol = "$" }) => (
  <div className="card p-5">
    <p className="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-2">{label}</p>
    <p className="text-2xl font-bold text-gray-900 dark:text-white">
      {typeof value === "number" ? `${currencySymbol}${value.toLocaleString("en-IE", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : value}
    </p>
    {trend && (
