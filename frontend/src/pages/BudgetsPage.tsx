...
const BudgetsPage: React.FC = () => {
  ...
  const {register,handleSubmit,reset,formState:{errors,isSubmitting}} = useForm<F>({resolver:zodResolver(schema),defaultValues:{month:now.getMonth()+1,year:now.getFullYear()}});

  const load = async () => { setLoading(true); try{const[b,c]=await Promise.all([api.get("/budgets"),categoryService.getAll()]);setBudgets(b.data);setCategories(c.data.filter((c:Category)=>c.type==="EXPENSE"));}finally{setLoading(false);} };
  useEffect(()=>{load();},[]);

  const onSubmit = async (data:F) => { try{await api.post("/budgets",data);toast.success("Budget set!");reset({month:now.getMonth()+1,year:now.getFullYear()});setShowModal(false);load();}catch(e:any){toast.error(e.response?.data?.error||"Failed");} };
  const handleDelete = async () => { if(!deleteBudget)return;await api.delete(`/budgets/${deleteBudget.id}`);toast.success("Deleted");setDeleteBudget(null);load(); };
  ...
  return (
    <div>
      ...
      {loading?<LoadingSpinner size="lg" className="py-32"/>:budgets.length===0?(
        <EmptyState icon={Target} title="No budgets set" description="Set spending limits per category to control your finances"/>
      ):(
          ...
          {budgets.map(b=>(
            <div key={b.id} className={clsx("card p-5 relative group",b.isOverBudget&&"border-red-200")}>
              ...
              <div className="flex items-center gap-3 mb-4">
                <div className="w-10 h-10 rounded-xl flex items-center justify-center text-xl flex-shrink-0" style={{backgroundColor:b.categoryColor+"20"}}>{b.categoryIcon}</div>
      ...
      {showModal&&(
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-sm animate-fade-in">
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100"><h2 className="text-base font-semibold">Set Budget</h2><button onClick={()=>setShowModal(false)} className="p-1.5 hover:bg-gray-100 rounded-lg"><X size={18}/></button></div>
            <form onSubmit={handleSubmit(onSubmit)} className="p-5 space-y-4">
              <div><label className="label">Expense Category</label><select {...register("categoryId",{valueAsNumber:true})} className="input"><option value="">Select...</option>{categories.map(c=><option key={c.id} value={c.id}>{c.icon} {c.name}</option>)}</select>{errors.categoryId&&<p className="text-xs text-red-500 mt-1">Required</p>}</div>
              ...
              <div className="flex gap-3 pt-1"><button type="button" onClick={()=>setShowModal(false)} className="btn-secondary flex-1">Cancel</button><button type="submit" disabled={isSubmitting} className="btn-primary flex-1">{isSubmitting?<LoadingSpinner size="sm"/>:"Set Budget"}</button></div>
            </form>
          </div>
        </div>
      )}
