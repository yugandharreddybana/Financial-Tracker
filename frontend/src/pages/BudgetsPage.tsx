...
const BudgetsPage: React.FC = () => {
  ...
  const load = async () => {
    ...
    try {
      ...
      setBudgets(Array.isArray(rawBudgets) ? rawBudgets : Array.isArray(rawBudgets?.content) ? rawBudgets.content : []);
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {safeBudgets.map(b => (
            <div key={b.id} className={clsx("card p-5 relative group", b.isOverBudget && "border-red-200")}>
              ...
              <button onClick={() => setDeleteBudget(b)} className="absolute top-3 right-8 p-1.5 text-slate-300 hover:text-red-500 hover:bg-red-50 rounded-lg opacity-0 group-hover:opacity-100 transition-all"><Trash2 size={14} /></button>
              <div className="flex items-center gap-3 mb-4">
                ...
                <div><p className="font-semibold text-slate-900 dark:text-slate-50 text-sm">{b.categoryName}</p><p className="text-xs text-slate-400">{MONTHS[b.month - 1]} {b.year}</p></div>
              </div>
              <div className="space-y-2">
                <div className="flex justify-between text-sm"><span className="text-slate-500">Spent</span><span className={clsx("font-bold", b.isOverBudget ? "text-red-600" : "text-slate-900 dark:text-slate-50")}>{b.percentage.toFixed(0)}%</span></div>
                <div className="h-2.5 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden"><div className={clsx("h-full rounded-full transition-all", getBarColor(b.percentage, b.isOverBudget))} style={{ width: `${Math.min(b.percentage, 100)}%` }} /></div>
                <div className="flex justify-between text-xs">
                  <span className={clsx("font-medium", b.isOverBudget ? "text-red-500" : "text-slate-500")}>€{Number(b.spentAmount).toFixed(2)} spent</span>
                  <span className="text-slate-400">€{Number(b.limitAmount).toFixed(2)} limit</span>
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white dark:bg-slate-950 rounded-2xl shadow-xl w-full max-w-sm animate-fade-in">
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 dark:border-slate-800">
              <h2 className="text-base font-semibold text-slate-900 dark:text-slate-50">Set Budget</h2>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="p-5 space-y-4">
              <div><label className="label">Expense Category</label>
                <select {...register("categoryId", { valueAsNumber: true })} className="input">
                  ...
                  {safeCats.map(c => <option key={c.id} value={c.id}>{c.icon} {c.name}</option>)}
                </select>
              ...
              <div className="flex gap-3 pt-1">
                ...
                <button type="submit" disabled={isSubmitting} className="btn-primary flex-1">{isSubmitting ? <LoadingSpinner size="sm" /> : "Set Budget"}</button>
              </div>
            </form>
          </div>
        </div>
