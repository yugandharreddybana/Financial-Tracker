...
const CategoriesPage: React.FC = () => {
  ...
  const onSubmit = async (data: F) => {
    try {
      ...
      reset({ type: "EXPENSE", icon: "🎯", color: "#3B82F6" });
      setShowModal(false);
      load();
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-3">
        {cats.map(cat => (
          <div key={cat.id} className="card p-3.5 relative group flex flex-col items-center text-center gap-1.5">
            ...
            <p className="text-xs font-medium text-slate-700 dark:text-slate-200 leading-tight">{cat.name}</p>
          </div>
        ))}
      </div>
    </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white dark:bg-slate-950 rounded-2xl shadow-xl w-full max-w-sm animate-fade-in max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 dark:border-slate-800 sticky top-0 bg-white dark:bg-slate-950">
              <h2 className="text-base font-semibold text-slate-900 dark:text-slate-50">New Category</h2>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {ICONS.map(i => (
                    <button type="button" key={i} onClick={() => setValue("icon", i)}
                      className={`w-9 h-9 rounded-xl text-lg flex items-center justify-center transition-all ${selIcon === i ? "ring-2 ring-indigo-500 bg-indigo-50" : "hover:bg-slate-100"}`}>{i}</button>
                  ))}
                </div>
              </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {COLORS.map(c => (
                    <button type="button" key={c} onClick={() => setValue("color", c)}
                      className={`w-8 h-8 rounded-lg ${selColor === c ? "ring-2 ring-offset-2 ring-slate-400" : ""}`} style={{ backgroundColor: c }} />
                  ))}
                </div>
              </div>
