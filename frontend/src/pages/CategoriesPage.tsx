...
const CategoriesPage: React.FC = () => {
  ...
  const {register,handleSubmit,watch,setValue,reset,formState:{errors,isSubmitting}} = useForm<F>({resolver:zodResolver(schema),defaultValues:{type:"EXPENSE",icon:"🎯",color:"#3B82F6"}});
  const selColor=watch("color"); const selIcon=watch("icon"); const selType=watch("type");

  const load = async () => { setLoading(true);try{const{data}=await categoryService.getAll();setCategories(data);}finally{setLoading(false);} };
  useEffect(()=>{load();},[]);

  const onSubmit = async (data:F) => { try{await categoryService.create(data);toast.success("Category added!");reset({type:"EXPENSE",icon:"🎯",color:"#3B82F6"});setShowModal(false);load();}catch(e:any){toast.error(e.response?.data?.error||"Failed");} };
  const handleDelete = async () => { if(!deleteCat)return;await categoryService.delete(deleteCat.id);toast.success("Deleted");setDeleteCat(null);load(); };
  ...
  return (
    <div>
      ...
      {loading?<LoadingSpinner size="lg" className="py-32"/>:categories.length===0?<EmptyState icon={Tag} title="No categories" description="Add your first category"/>:(
      ...
      {showModal&&(
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-sm animate-fade-in max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 sticky top-0 bg-white"><h2 className="text-base font-semibold">New Category</h2><button onClick={()=>setShowModal(false)} className="p-1.5 hover:bg-gray-100 rounded-lg"><X size={18}/></button></div>
            <form onSubmit={handleSubmit(onSubmit)} className="p-5 space-y-4">
              ...
              <div><label className="label">Icon</label><div className="flex flex-wrap gap-1.5">{ICONS.map(i=><button type="button" key={i} onClick={()=>setValue("icon",i)} className={`w-9 h-9 rounded-xl text-lg flex items-center justify-center transition-all ${selIcon===i?"ring-2 ring-primary-500 bg-primary-50":"hover:bg-gray-100"}`}>{i}</button>)}</div></div>
              <div><label className="label">Color</label><div className="flex flex-wrap gap-2">{COLORS.map(c=><button type="button" key={c} onClick={()=>setValue("color",c)} className={`w-8 h-8 rounded-lg ${selColor===c?"ring-2 ring-offset-2 ring-gray-400":""}`} style={{backgroundColor:c}}/>)}</div></div>
              <div className="flex gap-3 pt-1"><button type="button" onClick={()=>setShowModal(false)} className="btn-secondary flex-1">Cancel</button><button type="submit" disabled={isSubmitting} className="btn-primary flex-1">{isSubmitting?<LoadingSpinner size="sm"/>:"Add"}</button></div>
            </form>
          </div>
        </div>
      )}
