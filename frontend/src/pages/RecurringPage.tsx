...
const RecurringPage: React.FC = () => {
  ...
  const {register,handleSubmit,watch,reset,formState:{errors,isSubmitting}} = useForm<F>({resolver:zodResolver(schema),defaultValues:{type:"EXPENSE",frequency:"MONTHLY"}});
  const selType = watch("type");
  const filteredCats = categories.filter(c => c.type === selType);

  const load = async () => { setLoading(true); try{const[r,c]=await Promise.all([recurringService.getAll(),categoryService.getAll()]);setRecurrings(r.data);setCategories(c.data);}finally{setLoading(false);} };
  useEffect(()=>{load();},[]);

  const onSubmit = async (data:F) => { try{await recurringService.create(data);toast.success("Recurring added!");reset();setShowModal(false);load();}catch{toast.error("Failed");} };
  const handleDelete = async () => { if(!deleteR)return;await recurringService.delete(deleteR.id);toast.success("Deleted");setDeleteR(null);load(); };
  const handleProcess = async () => { try{const{data}=await recurringService.processDue();toast.success(`${data.processed} transactions processed!`);}catch{toast.error("Failed");} };
  ...
  return (
    <div>
      ...
      {loading?<LoadingSpinner size="lg" className="py-32"/>:recurrings.length===0?(
      ...
      {showModal&&(
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-sm animate-fade-in max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 sticky top-0 bg-white"><h2 className="text-base font-semibold">Add Recurring</h2><button onClick={()=>setShowModal(false)} className="p-1.5 hover:bg-gray-100 rounded-lg"><X size={18}/></button></div>
            <form onSubmit={handleSubmit(onSubmit)} className="p-5 space-y-4">
              ...
              <div><label className="label">Category</label><select {...register("categoryId",{valueAsNumber:true})} className="input"><option value="">Select...</option>{filteredCats.map(c=><option key={c.id} value={c.id}>{c.icon} {c.name}</option>)}</select>{errors.categoryId&&<p className="text-xs text-red-500 mt-1">Required</p>}</div>
